function Bcoils = get_Bcoil(coord_ext,Bcoils, fname, c, n)
    ...
    for i = 1:numel(x)
        ...
        for j = 1:numel(y)
            coord_B.Z = y(j);
    %       % Numerical approach
            [Br_loops_num, Bz_loops_num, Br_num(i,j), Bz_num(i,j)] = B_field(coord_B, coord_ext, coord_ext.Id, c, 'numerical', n);
    %       % Analytical approach (using Elliptic integrals)
            [Br_loops_ell, Bz_loops_ell, Br_ell(i,j), Bz_ell(i,j)] = B_field(coord_B, coord_ext, coord_ext.Id, c, 'analytical', n);
        end   
    end
